      ...
      USE globals, ONLY: p,nqpta,wpta,qpta
      ...
      SELECT CASE( 2*p ) 
        CASE( 1 )
          ...
          ALLOCATE(qpta(nqpta,2),STAT = alloc_status)
          IF(alloc_status /= 0) THEN
            PRINT*, 'Allocation error: qpta'
          ENDIF 

          ALLOCATE(wpta(nqpta),STAT = alloc_status)
          IF(alloc_status /= 0) THEN
      ...
      PRINT "(A,I3)", "Number of area quadrature points",nqpta

      PRINT "(A)", 'Area quadrature weights and points (r,s)'
      DO pt = 1,nqpta
        PRINT "(3(F10.3))", wpta(pt),qpta(pt,1),qpta(pt,2)
      ENDDO
      PRINT*, ' '
