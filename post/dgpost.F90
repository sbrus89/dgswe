      ...
      DO pe = 1,npe
        ...
        CALL read_solution(pe_direc,'hb.sol',t_sol,hb_global,lel2gel)
        CALL read_solution(pe_direc,'Z.sol',t_sol,Z_global,lel2gel)       
        CALL read_solution(pe_direc,'Qx.sol',t_sol,Qx_global,lel2gel)           
        CALL read_solution(pe_direc,'Qy.sol',t_sol,Qy_global,lel2gel)        
        ...
        IF (.not. ALLOCATED(Zsta_global)) THEN
          ...
          ALLOCATE(Zsta_global(nsta,nout_sta+1))
          ALLOCATE(Qxsta_global(nsta,nout_sta+1))
          ALLOCATE(Qysta_global(nsta,nout_sta+1))
          ALLOCATE(hbsta_global(nsta,1))
        ENDIF
        
        CALL read_stations(pe_direc,'hb.sta',t_sta,hbsta_global,sta_l2g)
        CALL read_stations(pe_direc,'Z.sta',t_sta,Zsta_global,sta_l2g) 
        CALL read_stations(pe_direc,'Qx.sta',t_sta,Qxsta_global,sta_l2g) 
        CALL read_stations(pe_direc,'Qy.sta',t_sta,Qysta_global,sta_l2g)      
